...
:rev-SparkMax: https://codedocs.revrobotics.com/cpp/classrev_1_1spark_1_1_spark_max.html[rev::spark::SparkMax^]
:pos-motor-pwm: 0
:pos-motor-quadA: 1
:pos-motor-quadB: 2
:pos-motor-abs: 0
...
void Robot::RobotInit() {
  frc::SmartDashboard::PutData( "Motor", &m_motor );
...
. It is dangerous to allow critical constants to be changed while in `TeleOperated` mode during a competition.  Competition robot code does not allow the user to change the gain constants.
|===
...
The {rev-SparkMax} class is used to communicate with the {hw-sparkmax} controller.  The {rev-SparkMax} is not part of the WPILib library and was added when the REVLib vendor library was added to the project in the steps above.  Therefore the documention for the {rev-SparkMax} class and other classes that are provided by the REVLib library are located on the RevRobotics website.  The xref:CodingGuide.adoc#resources[Resources] section of the Coding Guide gives links to the RevLib documentation, RevLib {CPP} API, and RevLib Examples. The xref:CodingGuide.adoc#resources[Resources] section also has several other useful links.

=== Velocity P-Control

NOTE: Read the xref:CodingGuide.adoc#motion-control[Motion Control] section (again).  

The {hw-sparkmax} is assigned CAN id #{spark-max-CANid} on the CAN bus. In `Robot.h`, create a variable for the motor using the {rev-SparkMax} class.  You will need to determine what header file is needed to use the class.  The {rev-SparkMax} `Set()` method will be used to control the motor percent output.  Use the {rev-SparkMax} `GetEncoder()` method to access the built-in encoder on the {hw-neo550}. The `GetEncoder()` method returns a https://codedocs.revrobotics.com/cpp/classrev_1_1spark_1_1_spark_relative_encoder[rev::SparkRelativeEncoder^] class object that can be used to retrieve the motor velocity (what method?).  Some conversion may be necessary if a gearbox is attached to the {hw-neo550}.
...
|===
a| `*TASK {counter:tasknum}*`
| Write a command based program to control the velocity motor. When the `A` button is pressed, stop the motor.  When the `B` button is pressed, set the motor speed to 1000 rpm.  When the `Y` button is pressed, set the motor speed to 2000 rpm.  Use `frc2::cmd::Run` with lambda expressions.  You will need to create a subsystem for the velocity motor and provide some methods to set the speed of the motor.  Use the feedforward and feedback (PID) code from the <<velocity-control-exercise>> in the `Periodic()` function of the subsystem.
a| QUESTIONS: ::
