...
|===
| `*TASK {counter:tasknum}*`
| Modify the code so the encoder returns distance in rotations by adding the following to the `RobotInit()` function:
|===
...
|===
| `*TASK {counter:tasknum}*`
| Modify the code so the encoder returns distance in degrees.
|===
...
|===
| `*TASK {counter:tasknum}*`
| Implement the above pseudocode for the P-controller. Change the value of the *K~p~* constant and observe the difference in behavior of the mechanism.
|===
...
|===
| `*TASK {counter:tasknum}*`
| Graph the desired position and the actual encoder position in `Shuffleboard` as you move from the 0 position to the 500 position and back.  Note whether the actual position gets exactly to the desired position.
|===
...
Relative encoders consider the "0" position to be wherever the motor was when the encoder was turned on.  Therefore it is not possible to know where the "0" position is and it changes each time the robot is powered on.

|===
| `*TASK {counter:tasknum}*`
| Note where the encoder considers the zero positon.  Disable the robot and manually move the position motor to a new position then restart the robot code by going to `Diagnostics -> Restart Robot Code` in the Driver Station.  Rerun the code and notice that the 0 position is not in the same location as the last time the code was run.
|===
...
|===
| `*TASK {counter:tasknum}*`
| Modify the code to move to locations based on angles in degrees when the *A* and *B* Buttons are pressed.  You will need to determine how to configure the  https://github.wpilib.org/allwpilib/docs/release/cpp/classfrc_1_1_duty_cycle_encoder.html[`frc::DutyCycleEncoder`] class to return angles in degrees.
|===
...
|===
| `*TASK {counter:tasknum}*`
| Modify the <<position-control-exercise>> code to use a trapezoidal profile to move between a 0 degree position when *A* is held and 180 degrees when *B* is held.
|===
