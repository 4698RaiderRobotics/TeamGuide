...
The Test Bench uses a https://docs.wpilib.org/en/stable/docs/software/roborio-info/roborio-introduction.html[RoboRIO 1] and a https://docs.wpilib.org/en/stable/docs/controls-overviews/control-system-hardware.html#ctre-power-distribution-panel[CTRE Power Distribution Panel].  It has two motors attached to it, a brushed motor (a https://www.andymark.com/products/neverest-classic-40-gearmotor[NeveRest with 40:1 gearbox]) and a brushless motor (a https://www.revrobotics.com/rev-21-1651/[NEO 550]) connected to two different motor controllers (a https://docs.wpilib.org/en/stable/docs/controls-overviews/control-system-hardware.html#spark-motor-controller[SPARK] and a https://www.revrobotics.com/rev-11-2158/[SparkMAX]).  The https://docs.wpilib.org/en/stable/docs/controls-overviews/control-system-hardware.html#spark-motor-controller[SPARK] motor controller is controlled with PWM and the https://www.revrobotics.com/rev-11-2158/[SparkMAX] is controlled over the https://en.wikipedia.org/wiki/CAN_bus[CAN bus] of the https://docs.wpilib.org/en/stable/docs/software/roborio-info/roborio-introduction.html[RoboRIO 1]. 

The NeveRest motor that is connected to a large disc is used in the position control exercises and refered to as the "Position Control" motor. It is connected to the https://docs.wpilib.org/en/stable/docs/controls-overviews/control-system-hardware.html#spark-motor-controller[SPARK] motor controller which is wired into the roboRIO PWM channel {pos-motor-pwm}.  The position control motor has a https://store.ctr-electronics.com/srx-mag-encoder/[SRX Mag Encoder] attached to its output shaft.  The SRX is wired into the roboRIO DIO channels {pos-motor-quadA}, {pos-motor-quadB}, and {pos-motor-abs}. Channels {pos-motor-quadA} & {pos-motor-quadB} are the Quadrature A and B inputs and channel {pos-motor-abs} is the absolute PWM signal.

The NEO 550 motor that is mounted on the short bracket is used in the velocity control exercises and refered to as the "Velocity Control" motor. It is connected to the https://www.revrobotics.com/rev-11-2158/[SparkMAX] motor controller which is connected to the roboRIO CAN bus with ID {spark-max-canID}.  The NEO 550 has a built-in encoder.
...
So far we have been using the relative quadrature output of the https://store.ctr-electronics.com/srx-mag-encoder/[SRX Mag Encoder].  The encoder also has a PWM signal for absolute positioning (See https://store.ctr-electronics.com/content/user-manual/Magnetic%20Encoder%20User%27s%20Guide.pdf[SRX Mag Encoder Hardware Guide]). The https://github.wpilib.org/allwpilib/docs/release/cpp/classfrc_1_1_duty_cycle_encoder.html[`frc::DutyCycleEncoder`] class is used to interface with the PWM absolute position signal which is wired to the DIO channel {pos-motor-abs}.

|===
| `*TASK {counter:tasknum}*`
| Modify the code to read the https://store.ctr-electronics.com/srx-mag-encoder/[SRX Mag Encoder]'s absolute PWM signal using the `GetAbsolutePosition()` method of the https://github.wpilib.org/allwpilib/docs/release/cpp/classfrc_1_1_duty_cycle_encoder.html[`frc::DutyCycleEncoder`] class.  Move the motor disc by hand and note the values returned by the encoder.  Use the absolute PWM signal to always move to the same 0 position when the *A* Button is pressed on the controller. 
a| QUESTIONS: ::
...
Velocity control is typically used on flywheels for shooting mechanisms or sometimes for intake wheels for game pieces.  Although it may seem like percent output control is essentially velocity control, it is not due to the battery voltage dropping while the robot is running during a competition.
