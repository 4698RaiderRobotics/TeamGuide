...
Moving and aligning imported components is done by using joints between components.
...
. Learn how to position inserted designs into the correct location using joints.
...
* Bearing Charts (xref:docs/MiniBearingSizes.pdf[Miniature Bearings], xref:docs/BearingSizes.pdf[Standard Bearings])
...
* https://robotics.nasa.gov/nasa-rap-robotics-design-guide/[NASA RAP Robotics Design Guide^]
